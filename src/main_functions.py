...
from src.analysis import calculate_feature_importance, perform_extended_logistic_regression, perform_and_display_regression, plot_residual_diagnostics_for_model, run_regression_analysis, plot_lag_correlations, calculate_feature_importance, check_stationarity, reduce_multicollinearity
from src.visualization import plot_residual_diagnostics, plot_roc_curve, display_classification_report
from src.texts import limitations_text, ext_logReg_coef_text, ext_logRed_text, logReg_coef_text, logReg_text, heteroscedasticity_text, residual_text, coefficients_text_secondLR, coefficients_text_firstLR, autocorrelation_text, filtering_text
...
def categorize_and_process_features(merged_data):
    ...
    try:
        ...
        st.write("Critical High Autocorrelation Variables:", critical_high_vars)
        st.write("Applying differencing to critical variables...")
        merged_data = apply_differencing(merged_data, critical_high_vars)
        st.write("Differencing applied. New columns added:", [f'diff_{var}' for var in critical_high_vars])

        return {'critical_high_vars': critical_high_vars, 'moderate_vars': moderate_vars, 'low_vars': low_vars}
    except KeyError as e:
...
def check_stationarity_and_multicollinearity(merged_data, autocorrelation_vars):
    """Checks stationarity and multicollinearity."""
    try:
        test_vars = (
            [f'diff_{var}' for var in autocorrelation_vars['critical_high_vars']] +
            autocorrelation_vars['moderate_vars'] +
            autocorrelation_vars['low_vars']
        )
